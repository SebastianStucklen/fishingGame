...
class Particle:
	...
	updates particles.
	'''
	def update(self, screen, attributes, emitterPos, delta:float, velo : Vector2 = Vector2(0, 0)):
		if self.time > 0: self.time -= delta #reduces the lifetime of the particle. This should probably be set to delta and adjusted by irl time instead of frame time but whatever.
		else: self.time = 0
	...
	def applyAttributes(self, attributes):
		attributeFunctions = {
			...
			"randColorChoice" : self.randColorChoice,
			}
		defaultSettings = {
			"randYVelo" : 5,
			"randXVelo" : 5,
			...
			"randColorChoice" : [(0,0,0),(100,100,100),(200,200,200)],
	...
	def randColorChoice(self,settings):
		self.color = random.choice(settings)
...
class ParticleEmitter:
	...
	'''
	def update(self, screen, delta : float = 1, pos = None, velo : Vector2 = Vector2(0, 0)):
